...
import { CheckCircle, Circle, ArrowRight, Palette } from 'lucide-react';
...
export default function CssModule() {
    ...
    // We search the 'css' track
    const cssTrack = curriculum.find(c => c.id === 'css');

    if (cssTrack) {
        // Try to find if 'moduleId' is a Module ID
        const matchModule = cssTrack.modules.find(m => m.id === moduleId);
        if (matchModule) {
            // It IS a module ID, so default to first lesson
            ...
            currentModule = cssTrack.modules.find(m => m.lessons.some(l => l.id === moduleId));
        }
    }

    const data = cssCourseData[currentLessonId];

    // Redirect if it was a module ID link, so URL reflects the lesson
    useEffect(() => {
        if (moduleId !== currentLessonId && data) {
            navigate(`/modules/css/${currentLessonId}`, { replace: true });
        }
    ...
    if (!data || !currentModule) {
        return (
            <div className="p-8 text-center">
                ...
                <button onClick={() => navigate('/modules/0')} className="mt-4 text-blue-600 hover:underline">Back to Dashboard</button>
            </div>
        );
    }
    ...
    const renderSidebar = () => (
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm mb-8">
            <h3 className="font-bold text-slate-400 uppercase text-xs tracking-wider mb-4 border-b border-slate-100 pb-2 flex items-center gap-2">
                <Palette size={14} />
                {currentModule.title}
            </h3>
            <div className="space-y-1">
                {currentModule.lessons.map(lesson => {
                    ...
                    return (
                        <button
                            key={lesson.id}
                            onClick={() => navigate(`/modules/css/${lesson.id}`)}
        ...
        <div className="flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto p-6">
            ...
            <div className="flex-1 min-w-0">
                <LessonLayout
                    ...
                    language="css" // Enable CSS Mode
                    htmlContext={data.htmlContext /* Will use generic default if undefined */}
                    quiz={renderQuiz()}
                    challenge={
